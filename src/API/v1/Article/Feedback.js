...
Server.register("GET", "/api/v1/article/feedback", async (request, response) => {
    ...
    const existing = await Database.querySingleAsync(`SELECT id, positive FROM article_feedback WHERE article = ${Database.escape(article.id)} AND user = ${Database.escape(request.server.client.address)}`);

    if(!existing)
        await Database.querySingleAsync(`INSERT INTO article_feedback (article, user, positive, timestamp) VALUES (${Database.escape(article.id)}, ${Database.escape(request.server.client.address)}, ${Database.escape((positive == "true"))}, ${Database.escape(Date.now())})`);
    ...
    return await Database.querySingleAsync(`SELECT positive FROM article_feedback WHERE article = ${Database.escape(article.id)} AND user = ${Database.escape(request.server.client.address)}`);
});
